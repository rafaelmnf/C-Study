...
// Receber uma Fila e dividir em 2 pilhas: uma com maiores de 20 e outras com menores de 20
void divide20 (Fila* f, Pilha* pmaior, Pilha* pmenor) {
    while(f->ini != NULL) {
        if(f->ini->info > 20) {
            push(pmaior, RetiraFila(f));
        }
        else {
            push(pmenor, RetiraFila(f));
        }
    }
    free(f);
    printf("Maiores que 20: ");
    imprime(pmaior);
    printf("Menores que 20: ");
    imprime(pmenor);
}
